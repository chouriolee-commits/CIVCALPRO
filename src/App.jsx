...
const Icon = ({ name, size = 18 }) => {
  const icons = {
    home:      <path d="M3 9.5L12 3l9 6.5V20a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z" fill="none" stroke="currentColor" strokeWidth="2" strokeLinejoin="round"/>,
    folder:    <><rect x="2" y="7" width="20" height="14" rx="2" fill="none" stroke="currentColor" strokeWidth="2"/><path d="M2 7l4-4h6l2 2h8" fill="none" stroke="currentColor" strokeWidth="2"/></>,
    clock:     <><circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" strokeWidth="2"/><path d="M12 7v5l3 3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></>,
    chart:     <><rect x="3" y="12" width="4" height="8" rx="1" fill="currentColor"/><rect x="10" y="7" width="4" height="13" rx="1" fill="currentColor"/><rect x="17" y="3" width="4" height="17" rx="1" fill="currentColor"/></>,
    ...
    help:      <><circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" strokeWidth="2"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="17" r="1" fill="currentColor"/></>,
    bell:      <><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" fill="none" stroke="currentColor" strokeWidth="2"/></>,
    ...
    chevron:   <path d="M9 18l6-6-6-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>,
    back:      <path d="M15 18l-6-6 6-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>,
    save:      <><path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z" fill="none" stroke="currentColor" strokeWidth="2"/><polyline points="17 21 17 13 7 13 7 21" fill="none" stroke="currentColor" strokeWidth="2"/><polyline points="7 3 7 8 15 8" fill="none" stroke="currentColor" strokeWidth="2"/></>,
    trash:     <><polyline points="3 6 5 6 21 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6" fill="none" stroke="currentColor" strokeWidth="2"/><path d="M10 11v6M14 11v6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><path d="M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2" fill="none" stroke="currentColor" strokeWidth="2"/></>,
    ...
    calc:      <><rect x="4" y="2" width="16" height="20" rx="2" fill="none" stroke="currentColor" strokeWidth="2"/><line x1="8" y1="6" x2="16" y2="6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><circle cx="8" cy="11" r="1" fill="currentColor"/><circle cx="12" cy="11" r="1" fill="currentColor"/><circle cx="16" cy="11" r="1" fill="currentColor"/><circle cx="8" cy="15" r="1" fill="currentColor"/><circle cx="12" cy="15" r="1" fill="currentColor"/><circle cx="16" cy="15" r="1" fill="currentColor"/><circle cx="8" cy="19" r="1" fill="currentColor"/><circle cx="12" cy="19" r="1" fill="currentColor"/></>,
    user:      <><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" fill="none" stroke="currentColor" strokeWidth="2"/><circle cx="12" cy="7" r="4" fill="none" stroke="currentColor" strokeWidth="2"/></>,
    arrow_r:   <path d="M5 12h14M12 5l7 7-7 7" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>,
  };
  return (
...
// ─── DATA ─────────────────────────────────────────────────────
const MODULES = [
  { id: 1, key: "computos",   label: "Cómputos Métricos",       desc: "Calcula cantidades de obra a partir de las dimensiones de tu proyecto.", icon: "📐", color: "card-green",  btnColor: "btn-green"  },
  { id: 2, key: "concreto",   label: "Dosificación de Concreto", desc: "Calcula la proporción de materiales para el concreto que necesitas.",     icon: "🚧", color: "card-orange", btnColor: "btn-orange" },
  { id: 3, key: "biblioteca", label: "Biblioteca de Materiales", desc: "Consulta información técnica de materiales de construcción y sus propiedades.", icon: "📚", color: "card-purple", btnColor: "btn-purple" },
  { id: 4, key: "estimacion", label: "Estimación de Materiales", desc: "Obtén la cantidad y costo de materiales necesarios para tu proyecto.",     icon: "🧮", color: "card-blue",   btnColor: "btn-blue"   },
];
    ...
    <aside className="sidebar desktop-only">
      <div className="sidebar-logo">
        ...
        <div className="logo-text">
          <strong>CIVCA<span>LPRO</span></strong>
      ...
      <nav className="sidebar-nav">
        ...
        >
          <span className="item-icon"><Icon name="home" size={15}/></span>
          Inicio
        </button>
        ...
        <button className="sidebar-item"><span className="item-icon"><Icon name="folder" size={15}/></span>Proyectos</button>
        <button className="sidebar-item"><span className="item-icon"><Icon name="clock"  size={15}/></span>Historial de Cálculos</button>
        <button className="sidebar-item"><span className="item-icon"><Icon name="chart"  size={15}/></span>Reportes</button>

        <div className="sidebar-section-label">Ajustes</div>
        <button className="sidebar-item"><span className="item-icon"><Icon name="settings" size={15}/></span>Configuración</button>
        <button className="sidebar-item"><span className="item-icon"><Icon name="help"     size={15}/></span>Ayuda</button>
      </nav>
...
function DesktopHome({ onNavigate }) {
  ...
  const volUnitario = (parseFloat(largo)||0) * (parseFloat(ancho)||0) * (parseFloat(altura)||0);
  const volTotal    = volUnitario * (parseFloat(cantidad)||0);

  return (
    <div className="page">
      <div className="page-greeting">
        <h1>¡Bienvenido a <span>CIVCALPRO</span>!</h1>
      ...
      <div className="module-grid">
        {MODULES.map(m => (
          <div key={m.id} className={`module-card ${m.color}`} onClick={() => onNavigate(m.key)}>
      ...
      <div className="module-preview">
        ...
        <div className="stepper-sidebar">
          ...
          {PASOS.map(p => (
            <div key={p.num} className={`stepper-step ${p.num === 1 ? "active" : ""}`}>
        ...
        <div className="preview-content">
          ...
          <div className="form-grid form-grid-4">
            <div className="form-field">
              <label>Largo (m)</label>
              <input type="number" value={largo} step="0.01" onChange={e => setLargo(e.target.value)}/>
            </div>
            <div className="form-field">
              <label>Ancho (m)</label>
              <input type="number" value={ancho} step="0.01" onChange={e => setAncho(e.target.value)}/>
            </div>
            <div className="form-field">
              <label>Altura (m)</label>
              <input type="number" value={altura} step="0.01" onChange={e => setAltura(e.target.value)}/>
            </div>
            <div className="form-field">
          ...
          <div className="result-bar">
            <span className="result-bar-left">
              Volumen unitario:&nbsp;&nbsp;<strong>{volUnitario.toFixed(2)} m³</strong>
            </span>
            <span className="result-bar-right">Volumen total: {volTotal.toFixed(2)} m³</span>
          </div>
  ...
  return (
    <>
      <header className="mobile-topbar">
        <button className="menu-btn"><Icon name="menu" size={22}/></button>
        <div className="logo-row">
          <span style={{fontSize:24}}>🏗️</span>
          <strong>CIVCA<span>LPRO</span></strong>
        </div>
        <button className="notif-btn" style={{background:"none",border:"none",cursor:"pointer",position:"relative"}}>
          <Icon name="bell" size={22}/>
          <span className="notif-badge">3</span>
        </button>
      </header>
      ...
      <div className="mobile-page">
        ...
        <div className="mobile-module-list">
          {MODULES.map(m => {
            ...
            return (
              <div key={m.id} className="mobile-module-card" onClick={() => onNavigate(m.key)}>
                <div className="mobile-module-card-icon" style={{background: bgs[m.id-1]}}>
                  {m.icon}
                </div>
                ...
                >{m.id}</span>
                <span className="mobile-chevron"><Icon name="chevron" size={16}/></span>
              </div>
            );
          })}
        </div>
        ...
        <div className="recent-section">
          <div className="recent-header">
            <div className="mobile-section-label" style={{margin:0}}>Proyectos Recientes</div>
            <span>Ver todos</span>
          </div>
  ...
  return (
    <>
      <header className="mobile-module-topbar">
        ...
        <button className="mobile-save-btn"><Icon name="save" size={22}/></button>
      </header>

      {/* Stepper */}
      <div className="mobile-stepper">
        {pasosMobile.map((label, i) => (
          <div key={i} className={`mobile-step ${i === 0 ? "active" : ""} ${i < 0 ? "done" : ""}`}>
      ...
      <div className="mobile-form-content">
        ...
        <div className="mobile-section-sub">Elige el tipo de elemento que deseas calcular</div>
        ...
        <div className="mobile-section-sub" style={{marginBottom:14}}>Ingresa las medidas requeridas</div>
      ...
      <div className="mobile-action-row">
        <button className="btn btn-ghost"><Icon name="trash" size={15}/> Limpiar</button>
        <button className="btn btn-green">Siguiente <Icon name="arrow_r" size={15}/></button>
      </div>
    </>
  );
}
    ...
    <>
      ...
      <div className="app-layout desktop-only">
        ...
        <div className="main-content">
          ...
          {activeModule === "inicio" && <DesktopHome onNavigate={handleNavigate}/>}
